...
1. User-submitted data
    ...
    - Email address (required for non-logged-in users and optional for OAuth login users): user's personal email address.
    ...
    - Personal homepage link (optional for both non-logged-in and OAuth login users)
...
| Email address | User input | Contact commenters and receive reply notifications | Cloudflare D1 database / localStorage | Required for non-logged-in users and optional for OAuth login users |
...
| Personal homepage link | User input | Public display in comment information | Cloudflare D1 database / localStorage | Optional for both non-logged-in and OAuth login users |
...
- Enhance user experience: Save user theme preference settings (light/dark mode) in localStorage to ensure users maintain their personalized interface settings on their next visit. For non-logged-in users, custom nickname and email address are saved in localStorage for subsequent comments.
...
- localStorage: User custom nickname, non-logged-in comment email, theme preference settings and comment drafts are stored in the browser's localStorage. Except when submitting comments, where nickname and email are sent to the server, this data is only stored on the user's local device. Users can clear this local data at any time through browser settings.
...
- Subscribe to feed.
- Post comments (non-logged-in users need to pass Cloudflare Turnstile human verification and provide a nickname and email address).
