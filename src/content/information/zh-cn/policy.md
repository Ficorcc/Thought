...
1. 用户提交数据
    - 自定义昵称（未登录用户必填）
    - 电子邮箱地址（未登录用户必填，OAuth 登录用户可选）：用户个人的电子邮箱地址。
    - 评论数据：包含评论内容及其发布时间戳。
    - 评论草稿：经用户编写但未提交的评论内容草稿。
    - Web Push 订阅数据：包含订阅端点信息。
    - OAuth 登录信息（用户授权登录）：包含第三方平台提供的用户名、显示名和头像及令牌数据。
    - 个人主页链接（未登录用户和 OAuth 登录用户均可选）
2. 自动收集数据
...
| 评论草稿 | 浏览器写入 | 恢复未完成的评论 | localStorage | 提交或清空文本框时将被删除 |
| 评论数据 | 用户输入 | 显示评论 | Cloudflare D1 数据库 | |
| Web Push 订阅数据 | 用户授权 | 发送桌面推送通知 | Cloudflare D1 数据库 | |
| 电子邮箱地址 | 用户输入 | 联系评论者、接收回复通知 | Cloudflare D1 数据库 / localStorage | 未登录用户必填，OAuth 登录用户可选 |
...
| 个人主页链接 | 用户输入 | 公开展示于评论信息 | Cloudflare D1 数据库 / localStorage | 未登录用户和 OAuth 登录用户均可选 |
...
- 提供评论服务：收集和处理用户评论数据，包括评论内容、评论草稿、发布时间、用户昵称或 OAuth 账户信息，以在站点上展示用户评论和互动内容。对于 OAuth 登录用户，还会显示其认证平台的用户名、显示名和头像信息。
- 增强用户体验：保存用户的主题偏好设置（浅色/深色模式）在 localStorage 中，确保用户下次访问时能够保持其个性化的界面设置。对于未登录用户，将自定义昵称和电子邮箱地址保存在 localStorage 中以便于后续评论。
- 通信与通知：使用 Web Push 订阅数据发送桌面推送通知，或使用电子邮箱地址发送邮件通知，及时告知用户有关其评论的回复或其他相关互动信息。
- 安全防护：使用 Cloudflare Turnstile 服务对未登录用户进行人机验证，防止评论滥用和恶意攻击。
- 内部分析与服务改进：利用 Cloudflare 提供的基础访问日志数据进行站点性能分析和安全监控，以改善服务质量和用户体验。

### 数据存储与保护

- Cloudflare D1：评论数据、OAuth 登录信息、个人主页链接、电子邮箱地址和 Web Push 订阅数据均存储在 Cloudflare D1 数据库中。该数据库提供企业级安全保护，包括数据加密存储、访问控制和定期备份。所有数据传输均通过加密连接进行。
...
- localStorage：用户的自定义昵称、未登录评论邮箱、主题偏好设置和评论草稿存储在浏览器的 localStorage 中。除提交评论时需要发送昵称和邮箱外，这些数据仅在用户本地设备上存储。用户可以随时通过浏览器设置清除这些本地数据。
...
- 浏览站点所有公开内容。
- 订阅 Feed。
- 发布评论（未登录用户需通过 Cloudflare Turnstile 人机验证，并提供昵称和电子邮箱地址）。
- 订阅和管理桌面推送通知。
